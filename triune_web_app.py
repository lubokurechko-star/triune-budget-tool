    ...
    if not uploaded_files:
        ...
        for idx, uploaded_file in enumerate(uploaded_files):
            ...
            with st.expander(f"📄 {uploaded_file.name}", expanded=True):
                ...
                with col1:
                    st.metric("Budget Revenue", budget_rev_str, key=key_br)
                    delta_color = "normal" if data['revenue_variance'] >= 0 else "inverse"
                    st.metric("Actual Revenue", actual_rev_str,
                             delta=rev_delta_str, delta_color=delta_color, key=key_ar)
                
                with col2:
                    st.metric("Budget Expenses", budget_exp_str, key=key_be)
                    delta_color = "inverse" if data['expense_variance'] >= 0 else "normal"
                    st.metric("Actual Expenses", actual_exp_str,
                             delta=exp_delta_str, delta_color=delta_color, key=key_ae)
                
                with col3:
                    st.metric("Budget Net", budget_net_str, key=key_bn)
                    delta_color = "normal" if data['net_variance'] >= 0 else "inverse"
                    st.metric("Actual Net", actual_net_str,
                             delta=net_delta_str, delta_color=delta_color, key=key_an)
                
                with col4:
                    st.metric("Budget Margin", budget_margin_str, key=key_bm)
                    delta_color = "normal" if margin_diff >= 0 else "inverse"
                    st.metric("Actual Margin", actual_margin_str,
                             delta=margin_delta_str, delta_color=delta_color, key=key_am)
