    ...
    if not uploaded_files:
        ...
        for idx, uploaded_file in enumerate(uploaded_files):
            ...
            with st.expander(f"📄 {uploaded_file.name}", expanded=True):
                ...
                with col1:
                    st.metric("Budget Revenue", f"${data['budget_revenue']:,.2f}", key=f"br_{idx}")
                    # Revenue: Less actual = BAD (red/inverse)
                    ...
                    st.metric("Actual Revenue", f"${data['actual_revenue']:,.2f}",
                             ...
                             delta_color=delta_color, key=f"ar_{idx}")
                
                with col2:
                    st.metric("Budget Expenses", f"${data['budget_expenses']:,.2f}", key=f"be_{idx}")
                    # Expenses: More actual = BAD (red/inverse)
                    ...
                    st.metric("Actual Expenses", f"${data['actual_expenses']:,.2f}",
                             ...
                             delta_color=delta_color, key=f"ae_{idx}")
                
                with col3:
                    st.metric("Budget Net", f"${data['budget_net']:,.2f}", key=f"bn_{idx}")
                    # Net: Less actual = BAD (red/inverse)
                    ...
                    st.metric("Actual Net", f"${data['actual_net']:,.2f}",
                             ...
                             delta_color=delta_color, key=f"an_{idx}")
                
                with col4:
                    st.metric("Budget Margin", f"{data['budget_margin']:.1f}%", key=f"bm_{idx}")
                    ...
                    st.metric("Actual Margin", f"{data['actual_margin']:.1f}%",
                             ...
                             delta_color=delta_color, key=f"am_{idx}")
